...
import DAV from '../../src/lib/DAV.js';

describe('DAV', () => {
  const getSpies = (SpyHTTPReturns = {}, SpyCacheReturns = {}) => {
    const SpyHTTP = jasmine.createSpyObj('HTTP', ['GET', 'HEAD', 'PUT', 'PROPFIND', 'DELETE', 'MKCOL', 'COPY', 'MOVE']),
    ...
    [
      ...
      ([object, returnValues]) => Object.entries(returnValues).forEach(
        ([method, returnValue]) => object[method] = object[method].and.returnValue(returnValue)
      )
    );
  ...
  it('should fire a COPY request on copy', () => {
    ...
    expect(SpyHTTP.COPY).toHaveBeenCalledWith('/copySource', {
      headers: {
        Destination: `${location.protocol}//${location.hostname}${location.port ? `:${location.port}` : ''}/copyDestination`
      }
    });
  });
  ...
  it('should fire a PROPFIND request and store cache on list', async () => {
    ...
    expect(SpyCache.set).toHaveBeenCalledWith('/checkPropfindRequest/', collection);
  });
  ...
  it('should fire a MOVE request on move', () => {
    ...
    expect(SpyHTTP.MOVE).toHaveBeenCalledWith('/moveSource', {
      headers: {
        Destination: `${location.protocol}//${location.hostname}${location.port ? `:${location.port}` : ''}/moveDestination`
      }
    });
  });
